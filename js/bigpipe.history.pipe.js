...
function load_chart()
{
    ...
    if($('#s2id_selPipe').css("display") != "none")
    {
        var selPipeName= $('#selPipe').find("option:selected").text();
    }
    else
    {
        var selPipeName = $('#inputPipe').attr("value");
    }
    ...
    var parse_string = "/history_data?type=pipe";
    parse_string += "&cluster_name=" + selClusterName;
    parse_string += "&pipe_name=" + selPipeName;
    ...
    if($('#option_pub_qps').attr('selected') == 'selected')
    {
        ...
        parse_string = "/chart_data?type=pipe&item=pub_qps";
        parse_string += "&cluster_name=" + selClusterName;
        parse_string += "&pipe_name=" + selPipeName;
        parse_string += "&stime=" + utime_start;
        parse_string += "&etime=" + utime_end;
    ...
    if($('#option_pub_flowrate').attr('selected') == 'selected')
    {
        ...
        parse_string = "/chart_data?type=pipe&item=pub_flowrate";
        parse_string += "&cluster_name=" + selClusterName;
        parse_string += "&pipe_name=" + selPipeName;
        parse_string += "&stime=" + utime_start;
        parse_string += "&etime=" + utime_end;
    ...
    if($('#option_sub_flowrate').attr('selected') == 'selected')
    {
        ...
        parse_string = "/chart_data?type=pipe&item=sub_flowrate";
        parse_string += "&cluster_name=" + selClusterName;
        parse_string += "&pipe_name=" + selPipeName;
        parse_string += "&stime=" + utime_start;
        parse_string += "&etime=" + utime_end;
...
$(document).ready(function(){
    ...
    $('#selCluster').change(function(){
        ...
        $('#selPipe').select2("enable", false);
        $('#submit').attr('disabled', true);
        $('#s2id_selPipe > a > span').html('正在读取列表...');
        ...
        xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState==4 && xmlhttp.status==200) {
                ...
                var ret = xmlhttp.responseText;
                var selPipe = document.getElementById('selPipe');
                selPipe.innerHTML = ret;

                if(selPipe.length > 0)
                {
                    selPipe[0].selected = true;
                    $('#s2id_selPipe > a > span').html(selPipe[0].value);

                    $('#selPipe').select2("enable", true);
                    $('#submit').attr('disabled', false);
                }
                else
                {
                    $('#s2id_selPipe > a > span').html('没有项目');
                    $('#submit').attr('disabled', true);
                }
            }
            //读取列表失败
        }
        var parse_string = "/history_pipe_list?cluster_id=" + selClusterId;
        xmlhttp.open("get", parse_string, true);
        xmlhttp.send();
    });
    ...
    $('#aCluster').click(function(){
        ...
        $('#s2id_selPipe').css("display", "none");
        $('#inputPipe').css("display", "block");
        $('#aPipe').css("display", "none");
        $('#submit').attr('disabled', false);
    });
    /*手动输入Pipe*/
    $('#aPipe').click(function(){
        $('#s2id_selPipe').css("display", "none");
        $('#inputPipe').css("display", "block");
        $('#aPipe').css("display", "none");
        $('#submit').attr('disabled', false);
